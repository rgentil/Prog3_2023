...
public class DFS<T> {
	 ...
	 */
	public DFS(Grafo<T> g) {
		this.grafo = g;
		this.colores = new HashMap<Integer, String>();
	}
	
	/**
	 * Inicializa el hash con los vertices todos en blanco. Key = vertice, Value un color.
	 * BLANCO = Estado inicial.
	 ...
	 * Complejidad  O(|V|+|A|). Va a pasar una vez por cada vertice y una vez por cada arco.
	 * Recorre todos los vertices que no hayan sido ni visitados ni tengo un estado final. O sea, que sea BLANCO.
	 * Entonces si inicia el recorrido desde ese vértice.
	...
	/**
	 * Se realiza el recorrido desde un vértice. Se coloca el estado parcial AMARILLO. Se agrega a la solución.
	 * Se recorren los vértices adyancente y se llama recursivo con cada vertices adyacente en estado inicial, BLANCO.
	 * Cuando se recorrieron todos los adyacentes de ese vertice (sale del for) el vertice se marca en NEGRO
